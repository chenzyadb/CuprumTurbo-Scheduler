...
configVersion 对应的APP版本: `1: V7.0.x; 2: V7.1.x; 3: V7.2.x; 4: V7.3.x`  
|字段            |类型   |定义               |
|:---------------|:------|:------------------|
|name            |string |配置文件的名称     |
...
例如SDM845为4+4设计，即`policy0: CPU0-3; policy1: CPU4-7`，将存在两个策略组.  
CPU数量上限为10个，策略组最大数量为10个，每个策略组最多包含10个CPU.  
|字段            |类型   |定义         |
|:---------------|:------|:------------|
|policyNum       |int    |策略组数量   |
#### 策略组参数  
用于生成CPU功耗模型和调度数据的必要参数，注意事项如下:  
...
|字段            |类型   |定义                                    |
|:---------------|:------|:---------------------------------------|
|cpus            |string |策略组中包含的CPU(格式:%d-%d)           |
|freqTablePath   |string |内核提供的CPU频率表地址                 |
|cpuCapacity     |int    |CPU算力量化值                           |
